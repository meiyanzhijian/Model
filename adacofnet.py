...
class KernelEstimation(torch.nn.Module):
    def __init__(self, kernel_size):
        ...
        self.moduleConv1 = RRCNN_block(ch_in=6, ch_out=32, t=2)
        self.modulePool1 = torch.nn.Sequential(
                        torch.nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, stride=2, padding=1),
                        torch.nn.ReLU(inplace=False),
                        )

        self.moduleConv2 = RRCNN_block(ch_in=32, ch_out=64, t=2)
        self.modulePool2 = torch.nn.Sequential(
                        torch.nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=2, padding=1),
                        torch.nn.ReLU(inplace=False),
                        )

        self.moduleConv3 = RRCNN_block(ch_in=64, ch_out=128, t=2)
        self.modulePool3 = torch.nn.Sequential(
                        torch.nn.Conv2d(in_channels=128, out_channels=128, kernel_size=3, stride=2, padding=1),
                        torch.nn.ReLU(inplace=False),
                        )

        self.moduleConv4 = RRCNN_block(ch_in=128, ch_out=256, t=2)
        self.modulePool4 = torch.nn.Sequential(
                        torch.nn.Conv2d(in_channels=256, out_channels=256, kernel_size=3, stride=2, padding=1),
                        torch.nn.ReLU(inplace=False),
                        )

        self.moduleConv5 = RRCNN_block(ch_in=256, ch_out=512, t=2)
        self.modulePool5 = torch.nn.Sequential(
        ...
        # self.se5 = SELayer(channel=512)

        self.moduleDeconv5 = RRCNN_block(ch_in=512, ch_out=512, t=2)
        self.moduleUpsample5 = Upsample(512)

        self.moduleDeconv4 = RRCNN_block(ch_in=512, ch_out=256, t=2)
        self.moduleUpsample4 = Upsample(256)

        self.moduleDeconv3 = RRCNN_block(ch_in=256, ch_out=128, t=2)
        self.moduleUpsample3 = Upsample(128)

        self.moduleDeconv2 = RRCNN_block(ch_in=128, ch_out=64, t=2)
    ...
    def forward(self, rfield0, rfield2):
        ...
        #tensorSe1 = self.se1(tensorConv1) + tensorConv1
        #tensorPool1 = self.modulePool1(tensorSe1)
        tensorPool1 = self.modulePool1(tensorConv1)

        tensorConv2 = self.moduleConv2(tensorPool1)
        #tensorSe2 = self.se2(tensorConv2) + tensorConv2
        #tensorPool2 = self.modulePool2(tensorSe2)
        tensorPool2 = self.modulePool2(tensorConv2)

        tensorConv3 = self.moduleConv3(tensorPool2)
        #tensorSe3 = self.se3(tensorConv3) + tensorConv3
        #tensorPool3 = self.modulePool3(tensorSe3)
        tensorPool3 = self.modulePool3(tensorConv3)

        tensorConv4 = self.moduleConv4(tensorPool3)
        #tensorSe4 = self.se4(tensorConv4) + tensorConv4
        #tensorPool4 = self.modulePool4(tensorSe4)
        tensorPool4 = self.modulePool4(tensorConv4)

        tensorConv5 = self.moduleConv5(tensorPool4)
        #tensorSe5 = self.se5(tensorConv5) + tensorConv5
        #tensorPool5 = self.modulePool5(tensorSe5)
