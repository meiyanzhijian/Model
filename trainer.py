...
########
    def test(self):
        # Test
##加入optimizer,scheduler
        torch.save({'epoch': self.current_epoch, 'state_dict': self.model.get_state_dict(),
...
#####################
        with torch.no_grad():
            self.model.eval()
########添加返回  psnr=   ,return
            psnr,ssim=self.test_loader.Test(self.model, self.result_dir, self.current_epoch, self.logfile, str(self.current_epoch).zfill(3) + '.png')
            self.logfile.write('\n')
        return psnr,ssim
########
    def terminate(self):
